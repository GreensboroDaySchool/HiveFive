...
class ThemesCollectionViewController: UICollectionViewController {
    ...
    private func preloadRenderedThemes() {
        Theme.glossary.enumerated().map{(IndexPath(row: $0.offset, section: 0), $0.element)}
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let cachedImg = cached[indexPath] {
            ...
            prepareCell(cell, theme: Theme.glossary[indexPath.row])
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let theme = Theme.glossary[indexPath.row]
...
struct Theme {
    ...
    static func decode(_ name: String) -> Theme {
        return glossary.filter{$0.name == name}[0]
    }
    
    static let glossary: [Theme] = [
